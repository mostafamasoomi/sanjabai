"""``/me/usage*`` endpoints -- per-user usage summary, enhanced usage (charts,
date filtering, top call), and CSV export. These are NOT admin-only; they
are gated on a regular logged-in user via ``_get_user_id``.
