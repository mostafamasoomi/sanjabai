...
async def reset_password(payload: ResetPasswordRequest) -> JSONResponse:
    ...
    if not uid:
        return err('توکن بازیابی نامعتبر یا منقضی شده است', 'The reset token is invalid or has expired.', 400)
...
async def get_telegram_token(request: Request, payload: TelegramTokenRequest) -> JSONResponse:
    ...
    async with async_session() as session:
        ...
        user = res.fetchone()
        if not user:
            return err('متصل نشده', 'Not linked.', 404)
        if user.telegram_id != payload.telegram_id:
            return err('عدم تطابق شناسه تلگرام', 'Telegram ID mismatch.', 403)
        token = _gen_token()
...
async def send_welcome_email(request: Request) -> JSONResponse:
    ...
    async with async_session() as session:
        ...
        if not user or not user.email:
            return err('ایمیلی ثبت نشده است', 'No email is registered.', 400)
