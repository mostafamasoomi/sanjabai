"""Memory endpoints: CRUD for persistent memory entries."""
...
@router.get('/memories')
async def list_memories(request: Request, category: Optional[str] = None) -> JSONResponse:
...
async def count_memories(request: Request, category: Optional[str] = None) -> JSONResponse:
...
async def search_memories(request: Request, q: str = '') -> JSONResponse:
    ...
    async with async_session() as session:
        ...
        if q:
            base = base.where(UserMemory.content.ilike(f'%{_escape_like(q)}%'))
        res = await session.execute(base.order_by(UserMemory.created_at.desc()).limit(50))
        rows = [dict(r._mapping) for r in res.fetchall()]
    return JSONResponse(jsonable_encoder(rows))
