...
FLAGS: dict[str, FlagMeta] = {
    ...
    'smart_llm_router_enabled': FlagMeta(
        ...
        wired=True,
        wire_note=(
            'وصل است: services/smart_router_llm.py::llm_route() -- گیت دوم از سه گیت، '
            'از chat_smart_mode.py:116 (مسیر :147) صدا زده می‌شود.\n'
            '🔴 این کلید را روشن نکنید. اندازه‌گیری ۲۰۲۶-۰۸-۲۸: روتر با فلگ روشن '
            '۱۰۰٪ به قواعد برمی‌گردد، ولی هزینه‌اش را می‌دهید. علت: '
            'app_setting کلید smart_router_model را ندارد، پس ارزان‌ترین عضو استخر '
            'انتخاب می‌شود و آن امروز یک مدل reasoning است که کل بودجهٔ ۲۴ توکن را '
            'صرف فکرکردن می‌کند و content خالی برمی‌گرداند. تأخیر مقصر نیست '
            '(میانه ۰.۷۸ ثانیه در برابر سقف ۴ ثانیه). بالابردن بودجه هم درستش '
            'نمی‌کند (با ۲۰۰ توکن هم خالی است). تا وقتی مدل روتر صریحاً ثبت نشود '
            'و پروب پذیرش بدهد، روشن‌کردن یعنی یک فراخوان متر‌شده روی هر پیام هوشمند '
            'که جوابش همیشه دور ریخته می‌شود.\n'
            '(جملهٔ قبلی اینجا می‌گفت هیچ مسیر چتی llm_route را صدا نمی‌زند — آن '
            'از نشست ۲۵ به بعد غلط شده بود.)\n'
            'سه گیت باید هم‌زمان باز باشند -- همین فلگ، انتخاب صریح کاربر، '
