...
class TestChat:
    ...
    def test_chat_completions_basic(self, mock_async_session):
        ...
        with patch('chat._http.post') as mock_post:
    ...
    def test_list_models(self, mock_async_session):
        ...
        with patch('models.async_session') as mock_session:
            mock_session.return_value.__aenter__.return_value.execute.return_value.fetchall.return_value = [
                MagicMock(id='tencent-hy3', display_name='Tencent Hy3', availability='available')
            ]
...
def mock_async_session():
    ...
    with patch('database.async_session') as mock:
