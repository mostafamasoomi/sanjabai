...
export default function UsersSection() {
  ...
  // DELETE call 405'd every time, so "رفع مسدودیت" never did anything.
  const toggleBan = async (uid: number) => {
    try {
      ...
      toast(banned ? 'کاربر مسدود شد' : 'کاربر رفع مسدودیت شد', 'success')
    } catch (err) {
      toast(errMessage(err, 'خطا در تغییر وضعیت مسدودیت'), 'error')
    }
  }
  ...
  const saveUserEdit = async () => {
    ...
    try {
      ...
      // so every "کاربر ویرایش شد" toast was for a save that never landed.
      ...
      toast(errMessage(err, 'خطا در ویرایش کاربر'), 'error')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <SectionHeader
          ...
          subtitle={
            total > users.length
              ? `نمایش ${faNum(users.length)} کاربر از ${faNum(total)} کاربر ثبت‌نام شده`
              : `${faNum(total)} کاربر ثبت‌نام شده`
          }
        />
                      ...
                      <td className="p-3">
                        <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            ...
                            title={u.banned ? 'رفع مسدودیت' : 'مسدودسازی'}
                          >
          ...
          <div className="card relative w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              {/* Direct wallet-balance editing was removed from this modal: the
                  ...
                  see backend/admin_users.py). Use the drawer's کیف پول tab. */}
            </div>
