...
/root/sanjabai/scripts/update_models.log.
"""
...
CONFIG_PATH = Path("/root/sanjabai/backend/litellm_config.yaml")
DB_URL = os.getenv(
    "DATABASE_URL",
    "postgresql+asyncpg://sanjabai:sanjabai@127.0.0.1:5432/sanjabai",
)
...
def update_pricing_via_subprocess(models: list[dict], irr_rate: float) -> int:
    ...
    sql_file.write_text("\n".join(sql_lines))
    log.info("Wrote SQL to %s — run manually via: docker exec -i sanjabai-sanjabai_pg-1 psql -U sanjabai < %s", sql_file, sql_file)
    return len(models)
